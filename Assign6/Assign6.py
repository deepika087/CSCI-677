...
with open(cifar10_folder + validation_dataset[0], 'rb') as f0:
    c10_validation_dict = pickle.load(f0)
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        if step % display_step == 0:
            validation_accuracy = accuracy(validation_prediction.eval(), test_labels)
            message = "step {:04d} : loss is {:06.2f}, accuracy on training set {:02.2f} %, accuracy on test set {:02.2f} %".format(step, l, train_accuracy, validation_accuracy)
            print message
...
cm = confusion_matrix(y_true=np.argmax(test_labels, axis=1), y_pred=test_prediction)
for i in range(c10_num_labels):
    class_name = "({}) {}".format(i, test_l[i])
    print(cm[i, :], class_name)
class_numbers = [" ({0})".format(i) for i in range(test_l)]
